...
if ( ! class_exists( 'YITH_WC_Catalog_Mode_Premium' ) ) {
	...
	class YITH_WC_Catalog_Mode_Premium extends YITH_WC_Catalog_Mode {
		...
		public function get_custom_button_template() {
			...
			if ( apply_filters( 'ywctm_get_vendor_postmeta', yit_get_prop( $product, '_ywctm_exclude_button' ), $post_id, '_ywctm_exclude_button' ) == 'yes' ) {
				return;
			}

			if ( apply_filters( 'ywctm_get_vendor_postmeta', yit_get_prop( $product, '_ywctm_custom_url_enabled' ), $post_id, '_ywctm_custom_url_enabled' ) == 'yes' ) {

				$protocol = apply_filters( 'ywctm_get_vendor_postmeta', yit_get_prop( $product, '_ywctm_custom_url_protocol' ), $post_id, '_ywctm_custom_url_protocol' );
				$link     = apply_filters( 'ywctm_get_vendor_postmeta', yit_get_prop( $product, '_ywctm_custom_url_link' ), $post_id, '_ywctm_custom_url_link' );
				$target   = apply_filters( 'ywctm_get_vendor_postmeta', yit_get_prop( $product, '_ywctm_custom_url_link_target' ), $post_id, '_ywctm_custom_url_link_target' );
		...
		public function show_product_price( $price, $product ) {
			...
			if ( defined( 'WOOCOMMERCE_CHECKOUT' ) || defined( 'WOOCOMMERCE_CART' ) || apply_filters( 'ywctm_ajax_admin_check', is_admin(), $product ) ) {
				return $price;
			}
			...
			if ( $hide_price == 'yes' && $this->apply_catalog_mode( $product_id ) ) {
				...
				if ( current_filter() == 'woocommerce_get_price' || current_filter() == 'woocommerce_product_get_price' && ! (float) $price ) {

					if ( class_exists( 'YITH_Request_Quote_Premium' ) && get_option( 'ywraq_show_button_near_add_to_cart' ) == 'yes' ) {
						$value = 0;
					} else {
						$value = '';
					}
		...
		public function set_price_label( $price, $product ) {
			...
			$product_id       = yit_get_base_product_id( $product );
			$alternative_text = apply_filters( 'ywctm_get_vendor_postmeta', yit_get_prop( $product, '_ywctm_alternative_text' ), $product_id, '_ywctm_alternative_text' );
