					...
					'options' => array(
						...
						'custom_columns'   => array(
							...
							'column_custom_url' => function ( $item, $me ) {
								...
								if ( yit_get_prop( $product, '_ywctm_exclude_button' . $this->get_vendor_id() ) == 'yes' ) {
									return __( 'Excluded from custom button', 'yith-woocommerce-catalog-mode' );
								}

								$protocol        = yit_get_prop( $product, '_ywctm_custom_url_protocol' . $this->get_vendor_id() );
								$link            = yit_get_prop( $product, '_ywctm_custom_url_link' . $this->get_vendor_id() );
								$target          = yit_get_prop( $product, '_ywctm_custom_url_link_target' . $this->get_vendor_id() );
						...
						default:
							...
							if ( ! empty( $product_ids ) ) {
								...
								foreach ( $product_ids as $product_id ) {
									...
									$args    = array(
										'_ywctm_custom_url_enabled' . $this->get_vendor_id()     => $enable_url,
										'_ywctm_exclude_button' . $this->get_vendor_id()         => $exclude,
										'_ywctm_custom_url_protocol' . $this->get_vendor_id()    => $protocol,
										'_ywctm_custom_url_link' . $this->get_vendor_id()        => $link,
										'_ywctm_custom_url_link_target' . $this->get_vendor_id() => $target,
									);
			...
			if ( isset( $_GET['id'] ) && ! empty( $_GET['action'] ) && ( 'edit' == $_GET['action'] ) ) {
				...
				switch ( $current_section ) {
					...
					default:
						...
						$item          = array(
							'ID'                => $_GET['id'],
							'enable_custom_url' => yit_get_prop( $product, '_ywctm_custom_url_enabled' . $this->get_vendor_id() ),
							'exclude'           => yit_get_prop( $product, '_ywctm_exclude_button' . $this->get_vendor_id() ),
							'protocol'          => yit_get_prop( $product, '_ywctm_custom_url_protocol' . $this->get_vendor_id() ),
							'link'              => yit_get_prop( $product, '_ywctm_custom_url_link' . $this->get_vendor_id() ),
							'target'            => yit_get_prop( $product, '_ywctm_custom_url_link_target' . $this->get_vendor_id() ),
						);
