					...
					'options' => array(
						...
						'custom_columns'   => array(
							...
							'column_alternative_text' => function ( $item ) {
								...
								return yit_get_prop( $product, '_ywctm_alternative_text' . $this->get_vendor_id() );
							},
						),
						'bulk_actions'     => array(
							'actions'   => array(
						...
						default:
							...
							if ( ! empty( $product_ids ) ) {
								...
								foreach ( $product_ids as $product_id ) {
									...
									if ( $alternative_text ) {
										yit_save_prop( $product, '_ywctm_alternative_text' . $this->get_vendor_id(), $alternative_text, true );
									} else {
										yit_delete_prop( $product, '_ywctm_alternative_text' . $this->get_vendor_id() );
									}
			...
			if ( isset( $_GET['id'] ) && ! empty( $_GET['action'] ) && ( 'edit' == $_GET['action'] ) ) {
				...
				switch ( $current_section ) {
					...
					default:
						...
						$item          = array(
							'ID'               => $_GET['id'],
							'alternative_text' => yit_get_prop( $product, '_ywctm_alternative_text' . $this->get_vendor_id() ),
						);
